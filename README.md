...
## 开发笔记
[开发笔记](http://blog.wujinxing.top/2019/08/13/miao-sha-xiang-mu-bi-ji/#more "笔记")

## 开发、压测工具
IDEA + SQLyog + Git + Chrome + JMeter
...
1. 将请求尽量拦截在系统上游：数据库访问是瓶颈。传统秒杀系统之所以挂，请求都压倒了后端数据层，数据读写锁冲突严重，几乎所有请求都超时，流量虽大，下单成功的有效流量甚小，通过限流、降级等措施来最大化减少对数据库的访问，从而保护系统。
...
### 10. 自定义注解控制器实现限流、使用RateLimiter实现限流
描述：秒杀商品时，可能会因为访问量太高而导致系统崩溃，此时要使用限流来进行限制访问量，当达到限流阀值，后续请求会被降级；降级后的处理方案可以是：返回排队页面（高峰期访问太频繁，等一会重试）、错误页等。

实现1：加入拦截器AccessLimit，具体实现参考access包的三个类，其中AccessInterceptor需要加注解@Service，实现之后，还需要在config中的WebConfig中将AccessInterceptor注入进来，重写WebConfig继承的WebMvcConfigurerAdapter类的addInterceptors方法，将该拦截器添加进去，这样注解就会生效。
需要在哪个controller方法上进行限流，直接加注解并设置相应的参数就可以。

实现2：项目使用RateLimiter来实现限流，RateLimiter是guava提供的基于令牌桶算法的限流实现类，通过调整生成token的速率来限制用户频繁访问秒杀页面，从而达到防止超大流量冲垮系统。（令牌桶算法的原理是系统会以一个恒定的速度往桶里放入令牌，而如果请求需要被处理，则需要先从桶里获取一个令牌，当桶里没有令牌可取时，则拒绝服务）
